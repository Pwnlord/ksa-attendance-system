...
| DEC-007 | Authorized participant roster | DECIDED: Admin imports an approved CSV roster containing serial, name, and email/phone. Entries move from `UNCLAIMED` to `CLAIMED` after matching registration. Duplicate/disputed claims go to Admin. Entries may be disabled or corrected without deletion. First-claim registration is a pilot exception requiring Admin review. | Product Owner | Registration |
