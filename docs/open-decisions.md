...
| DEC-038 | Participant browser account binding | DECIDED: One browser installation may be bound to one Participant account at a time. Once a browser has a device credential, new Participant registration and login for a different Participant account are blocked; logging out preserves the binding so the same account can return. There is no browser-release action. Clearing cookies or using a new browser remains recoverable through the existing device-change approval flow. This is a browser-installation control, not immutable physical-device identification. | Product Owner/Security | Auth |
