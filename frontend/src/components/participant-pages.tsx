...
export function ParticipantProfile({ initialUser }: { initialUser: User }) {
  ...
  useEffect(load, []);
  async function requestDevice() { setSubmitting(true); setDeviceError(null); setMessage(""); try { await api.deviceRequest(note); setMessage("Your device-change request is waiting for review."); setNote(""); load(); } catch (reason) { setDeviceError(reason); } finally { setSubmitting(false); } }
