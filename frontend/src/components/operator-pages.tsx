...
function SessionRow({ session }: { session: AttendanceSession }) {
  return <div className="flex flex-col gap-2 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-ink">{sessionLabel(session)}</p><p className="mt-1 text-xs text-muted">{session.presentCount} present · version {session.version}</p></div><StatusPill status={session.status} /></div>;
}
...
export function SessionHistory() {
  ...
  useEffect(load, []);
  return <><PageHeading eyebrow="Operations" title="Session history" description="Review session timing, state, and present counts. Closed sessions are not reopened by Course Representatives." action={<Button variant="secondary" onClick={load}>Refresh</Button>} />{error ? <Notice tone="error">{apiErrorMessage(error)}</Notice> : items ? <Card><div className="divide-y divide-border">{items.length === 0 ? <EmptyState title="No sessions" description="Sessions created for the Academy will appear here." /> : items.map((session) => <SessionRow key={session.id} session={session} />)}</div></Card> : <LoadingBlock label="Loading session history…" />}</>;
}
...
export function ParticipantSearch() {
  ...
  async function search(event?: React.FormEvent) {
    ...
    setError(null);
    try { setItems((await api.participants(query)).items); } catch (reason) { setError(reason); }
  }
  return <><PageHeading eyebrow="Operations" title="Participant search" description="Find a participant by name or serial number. Only the minimum roster details needed for operations are shown." /><Card><form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={search}><Field className="flex-1" label="Name or serial number" name="participant-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="At least two characters" /><Button type="submit">Search</Button></form></Card>{error ? <div className="mt-5"><Notice tone="error">{apiErrorMessage(error)}</Notice></div> : null}{items ? <Card className="mt-5"><div className="divide-y divide-border">{items.length === 0 ? <EmptyState title="No participants found" description="Try a different name or serial number." /> : items.map((participant) => <div key={participant.id} className="flex flex-col gap-2 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{participant.fullName}</p><p className="mt-1 text-sm text-muted">{participant.serialNumber}</p></div><StatusPill status={participant.status} /></div>)}</div></Card> : null}</>;
}
