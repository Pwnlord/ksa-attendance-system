...
export function ParticipantSearch() {
  ...
  useEffect(() => { api.currentSession().then(setCurrentSession).catch(() => setCurrentSession(null)); }, []);
  ...
  return <><PageHeading eyebrow="Operations" title="Participant search" description="Find a participant by name or serial number. Only the minimum roster details needed for operations are shown." /><Card><form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={search}><Field className="flex-1" label="Name or serial number" name="participant-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="At least two characters" /><Button type="submit">Search</Button></form></Card>{error ? <div className="mt-5"><Notice tone="error">{apiErrorMessage(error)}</Notice></div> : null}{selectedParticipant ? <Card className="mt-5 border-amber-100 bg-amber-50/50"><h2 className="text-lg font-bold">Emergency attendance</h2><p className="mt-2 text-sm leading-6 text-muted">This creates a manual attendance record for the current session and is fully audited. The backend prevents recording your own attendance.</p><p className="mt-4 font-semibold">{selectedParticipant.fullName} · {selectedParticipant.serialNumber}</p><div className="mt-4 space-y-4"><TextArea label="Reason" name="emergency-reason" value={emergencyReason} onChange={(event) => setEmergencyReason(event.target.value)} placeholder="Explain why normal check-in was unavailable." /><div className="flex flex-wrap gap-3"><Button onClick={() => void recordEmergency()} disabled={emergencyBusy || emergencyReason.trim().length < 3}>Record attendance</Button><Button variant="secondary" onClick={() => { setSelectedParticipant(null); setEmergencyReason(""); }}>Cancel</Button></div></div></Card> : null}{items ? <Card className="mt-5"><div className="mb-4"><Notice tone={currentSession ? "info" : "warning"}>{currentSession ? `Current session: ${sessionLabel(currentSession)}. Emergency attendance is available for eligible participants.` : "There is no open session, so emergency attendance is unavailable."}</Notice></div><div className="divide-y divide-border">{items.length === 0 ? <EmptyState title="No participants found" description="Try a different name or serial number." /> : items.map((participant) => <div key={participant.id} className="flex flex-col gap-3 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{participant.fullName}</p><p className="mt-1 text-sm text-muted">{participant.serialNumber}</p></div><div className="flex items-center gap-3"><StatusPill status={participant.status} /><Button variant="secondary" onClick={() => setSelectedParticipant(participant)} disabled={!currentSession}>Emergency attendance</Button></div></div>)}</div></Card> : null}</>;
}
